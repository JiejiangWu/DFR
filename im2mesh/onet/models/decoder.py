...
class DecoderCBatchNorm(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128,
                 hidden_size=256, leaky=False, legacy=False,norm_method = 'batch_norm'):#instance_norm,group_norm
        super().__init__()
        self.z_dim = z_dim
        if not z_dim == 0:
            self.fc_z = nn.Linear(z_dim, hidden_size)
        ...
        if not legacy:
            self.bn = CBatchNorm1d(c_dim, hidden_size,norm_method)
        else:
            self.bn = CBatchNorm1d_legacy(c_dim, hidden_size,norm_method)
